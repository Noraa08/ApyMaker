...
export const deletePost = createAsyncThunk(
  ...
  async (initialPost: IPost) => {
    ...
    try {
      ...
      if (response?.status === 200) return initialPost; // only for jsonplaceholder fake api server
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(updatePost.fulfilled, (state, action) => {
        // if payload does not have the id property
        ...
        action.payload.date = new Date().toISOString(); // set a new date

        if (state.posts !== undefined) {
          const posts: IPost[] = state.posts.filter((post) => post.id !== id);
...
export const selectPostById = (state: RootState, postId: number) =>
  state.yumePosts.posts?.find((post) => post.id === postId);
